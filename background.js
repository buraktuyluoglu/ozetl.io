...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'getTranscript' || request.action === 'getSummary') {
        const endpoint = request.action === 'getTranscript' ? 'get_transcript' : 'summarize';
        ...
        .then(response => {
            if (!response.ok) {
                // Sunucudan gelen hatayı JSON olarak alıp content script'e gönder
